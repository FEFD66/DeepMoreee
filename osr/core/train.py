...
from osr.utils import AverageMeter


def train(net, criterion, optimizer, trainloader, epoch=None, use_gpu: bool = True, print_freq: int = 100):
    net.train()
    losses = AverageMeter()
...
def train_cs(net, netD, netG, criterion, criterionD, optimizer, optimizerD, optimizerG,
             trainloader,nz,ns,print_freq,beta:float=0.1, epoch=None, use_gpu=True,):
    ...
    for batch_idx, (data, labels) in enumerate(trainloader):
        ...
        noise = torch.FloatTensor(data.size(0), nz, ns, ns).normal_(0, 1).cuda()
        if use_gpu:
            noise = noise.cuda()
        ...
        generator_loss = errG + beta * errG_F
        ...
        total_loss = loss + beta * F_loss_fake
        total_loss.backward()
        optimizer.step()
